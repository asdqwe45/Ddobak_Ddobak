...
public class CartServiceImpl implements com.ddobak.cart.service.CartService {
    ...
    public List<FontCartResponse> getCart(LoginInfo loginInfo){
        ...
        for(Cart c : cart){
            ...
            FontCartResponse temp = new FontCartResponse(c.getFont().getId(),c.getFont().getProducer().getId(), c.getFont().getKorFontName(),c.getMember().getNickname(),favoriteCheck,c.getFont().getPrice(),c.getFont().getFont_file_url());
            result.add(temp);
        }
